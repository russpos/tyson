 ...
 */
var updateFileContents = function() {
    ...
    var checksum = -1;
    try {
        var oldContents = this.os.readFile(this.settings.path);
    ...
    if (checksum == md5(contents)) {
        ...
        // Write the file
        this.os.writeFile(this.settings.path, contents);
        return true;
    }
    return false;
};
 ...
 */
var updateFileOwnership = function() {
    var fileStats = this.os.fileStats(this.settings.path);
    var ownership = this.os.convertUserAndGroupToId(this.settings.owner,
                                                    this.settings.group);

    if (ownership[0] !== fileStats.uid || ownership[1] !== fileStats.gid) {
        Logger.debug("Changing ownership " + this.settings.path);
        // Set the owners
        this.os.chown(this.settings.path,
                ownership[0],
                ownership[1]);
        return true;
    }
 ...
 */
var updateFilePermissions = function() {

    // TODO -- Actually check the perms first
    this.os.chmod(this.settings.path,
             this.settings.permissions);
    return false;
};

var queueChangeEvent = function() {
    this.hasChanged = (this.contentsChanged || this.ownerChanged || this.permsChanged);
    reset.apply(this, [false, false]);
};

var reset = function(os, hostInfo) {
 ...
 * @return Bool Returns true if any change occurs 
*/
var ensureExists = function(chain) {
    return chain
    .then(updateFileContents.bind(this))
    .then(updateFileOwnership.bind(this))
    .then(updateFilePermissions.bind(this))
    .then(queueChangeEvent.bind(this))
};
 ...
 * @return Bool Returns true if any change occurs 
*/
var ensureRemoved = function(chain) {
    // TODO
    return chain;
};
 ...
 * @param Promise chain Promise to chain events on to
 * @return Bool Returns true if any change occurs 
 */
File.prototype.exec = function(os, hostInfo, chain) {
    Logger.debug(this.settings.path);
    reset.apply(this, [os, hostInfo]);

    if (this.settings.ensure == ENSURE.REMOVED) {
        return ensureRemoved.apply(this, [chain]);
    } else if (this.settings.ensure == ENSURE.EXISTS) {
        return ensureExists.apply(this, [chain]);
    }
};
